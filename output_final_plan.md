...
Validation update on 2026-09-20:

- Live authenticated API validation passed for admin statistics/reports and CSV export, poster and worker marketplace reads, notification writes, logout, reversible poster job create/cancel, worker bid and assignment acceptance, and the 11 MiB proof-upload boundary. The temporary validation jobs were canceled and verified in their terminal state.
- The live fraud settings match the documented baseline exactly: minimum description 20, daily velocity 10, shared identity threshold 2, review threshold 20, and explicit ban confirmation enabled.
- The configured scheduler command completed successfully, and one real SMTP message was accepted by the configured SMTP server. Mailbox receipt and hosting-cron execution remain external confirmations.
- The 11,534,336-byte valid proof reached the deployed application and was rejected by the intended 10 MiB application limit, confirming the hosting request ceiling is above that limit.
- The production database backup/control-plane item is not complete: the active local `.env` points to loopback, so its 27-table backup is not a verified live-host backup; FTP inspection found no `storage/queue` directory and only old scheduler logs. A hosting control-panel/SSH backup and queue/cron confirmation are still required.
- The approved ad provider, Android client codebase, publisher credentials, and compliance decision were not supplied; item 7 remains blocked at that exact external dependency.
